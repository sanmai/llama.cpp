...
if (type == GGML_TYPE_MXFP4) { // NVFP4 uses the generic q8_1 path (W4A8), not native fp4
...
static constexpr __host__ __device__ int mmq_get_mma_tile_x_k(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_NVFP4:   return MMQ_MMA_TILE_X_K_NVFP4; // generic q8_1 path (W4A8) on all archs
...
struct mmq_type_traits<mmq_x, mmq_y, need_check, GGML_TYPE_NVFP4> {
    // NVFP4 uses the generic q8_1 int8-MMA path (W4A8) on every arch, including Blackwell.
    // The native fp4 (W4A4) path is reserved for MXFP4; 4-bit activations are too lossy here.
...
static __device__ __forceinline__ void mul_mat_q_process_tile(
        ...
        const int tile_x_max_i, const int tile_y_max_j, const int kb0_start, const int kb0_stop, const float scale) {
...
#if defined(BLACKWELL_MMA_AVAILABLE)
    // FP4 tile stores 8 blocks (MXFP4 only; NVFP4 uses the q8_1 W4A8 path)
...
#pragma unroll
    ...
    // Scale the assembled tile sum by the optional per-tensor weight scale (NVFP4 weight_scale_2).
    // The caller passes the real scale only when this block computes the whole tile (kb0_start == 0);
    // split-tile partials get 1.0f and are scaled once in the stream-k fixup, so the sum is scaled exactly once.
    if (scale != 1.0f) {
#pragma unroll
...
#pragma unroll
        ...
        // Only a tile computed in full here (kb0_start == 0) is final; a tail of a split tile is
...
#pragma unroll
    ...
    // Partial (head) of a split tile: written unscaled; the stream-k fixup applies the scale once.
    mul_mat_q_process_tile<type, mmq_x, need_check, fixup>
        (x, offset_x, y + offset_y, ids_dst_shared, dst + offset_dst, tmp_fixup, stride_row_x, ncols_y, stride_col_dst,
         tile_x_max_i, tile_y_max_j, kb0_start, kb0_stop, 1.0f);
}
...
#pragma unroll
        for (int j0 = 0; j0 < mmq_x; j0 += nwarps) {
            ...
            dst[j*stride_col_dst + i] = (dst[j*stride_col_dst + i] + sum[j0/nwarps]) * scale;
        }
        return;
    }
...
#pragma unroll
    for (int j0 = 0; j0 < mmq_x; j0 += nwarps) {
        ...
        dst[ids_dst_shared[j]*stride_col_dst + i] = (dst[ids_dst_shared[j]*stride_col_dst + i] + sum[j0/nwarps]) * scale;
    }
}
...
struct mmq_args {
    ...
    const float * x_scale = nullptr; // optional per-tensor scale (NVFP4 weight_scale_2), applied to the output
};
