...
bool ggml_cuda_should_use_mmq(enum ggml_type type, int cc, int64_t ne11, int64_t n_experts) {
    ...
    // MEASUREMENT (revert after): force NVFP4 off MMQ onto the cuBLAS dequant path (W4A16) to
    // isolate weight-quant quality from the native FP4 W4A4 activation penalty.
    static const bool nvfp4_force_dequant = getenv("GGML_NVFP4_FORCE_DEQUANT") != nullptr;
    if (nvfp4_force_dequant && type == GGML_TYPE_NVFP4) {
        return false;
    }
