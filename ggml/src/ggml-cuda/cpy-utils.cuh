...
static __device__ void quantize_f32_nvfp4_block(const float * __restrict__ x, block_nvfp4 * __restrict__ y) {
    constexpr int test_offsets[5] = { 0, -1, 1, -2, 2 };
    ...
    for (int s = 0; s < QK_NVFP4/QK_NVFP4_SUB; ++s) {
        ...
        // Refine the UE4M3 sub-block scale: try first_code +/- {0,1,2} and keep the one
        // with the lowest summed squared reconstruction error (cf. quantize_mmq_nvfp4).
        ...
        for (int t = 0; t < 5; ++t) {
            ...
            float err = 0.0f;
            for (int j = 0; j < QK_NVFP4_SUB; ++j) {
                const uint8_t q = ggml_cuda_float_to_fp4_e2m1(xb[j], id);
                const float   e = fabsf(xb[j]) - kvalues_mxfp4[q & 0x7]*d;
                err = fmaf(e, e, err);
            }
            if (err < best_err) {
                best_err = err;
        ...
        y->d[s] = best_ue;
        const float id = best_d > 0.0f ? 0.5f/best_d : 0.0f;
