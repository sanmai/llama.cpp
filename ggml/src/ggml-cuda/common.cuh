...
static __device__ __forceinline__ float ggml_cuda_ue4m3_to_fp32(uint8_t x) {
    ...
    const uint32_t bits = x * (x != GGML_UE4M3_NAN_CODE && x != 0xFF); // Convert NaN to 0.0f to match CPU implementation.
    ...
    return static_cast<float>(xf) / 2;
#else
    if (x == 0 || (x == GGML_UE4M3_NAN_CODE && x != 0xFF)) { // Convert NaN to 0.0f
        return 0.0f;
    }
